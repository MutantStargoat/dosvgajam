...
static int next_token(struct parser *pst)
{
	...
	if(isalpha(c)) {
		...
		while((c = nextchar(pst)) != -1 && (isalnum(c) || c == '_' || c == ':' || c == '(' || c == ')')) {
			dynarr_strpush_nf(pst->token, c);
		}
		if(c != -1) ungetchar(c, pst);
		return TOK_ID;
