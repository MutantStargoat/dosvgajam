...
void json_value_int(struct json_value *jv, long num)
{
	jv->type = JSON_NUM;
	jv->num = (double)num;
	jv->inum = num;
...
long json_lookup_int(struct json_obj *obj, const char *path, long def)
{
	struct json_value *val;
...
static int next_token(struct parser *p)
{
	...
	case '"':
		dynarr_clear_nf(p->token);
		next_char(p);
		while(p->nextc && p->nextc != '"') {
			dynarr_strpush_nf(p->token, p->nextc);
			next_char(p);
		}
		next_char(p);
		return TOKEN_STR;
