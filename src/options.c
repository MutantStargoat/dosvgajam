...
#define DEF_SCALE		3
#define DEF_SCALER		SCALER_NEAREST
#define DEF_MOUSE_SPEED	50
#define DEF_LOGFILE		"game.log"
...
int load_options(const char *fname)
{
	...
	opt.logfile = strdup(ts_lookup_str(cfg, "options.system.logfile", DEF_LOGFILE));
...
int save_options(const char *fname)
{
	...
	fprintf(fp, "\tsystem {\n");
	WROPT(2, "logfile = %s", opt.logfile, DEF_LOGFILE);
	fprintf(fp, "\t}\n");
